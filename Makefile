...
prod-update: ## Update code from Git and restart services
	...
	@echo ""
	@echo "Checking if config/inertia.php exists..."
	-$(DOCKER_COMPOSE_PROD) exec -T app test -f config/inertia.php && echo "✅ config/inertia.php exists" || echo "❌ config/inertia.php NOT FOUND!"
	@echo ""
	@echo "Laravel config values (checking for errors):"
	-$(DOCKER_COMPOSE_PROD) exec -T app php -r "try { \$$enabled = config('inertia.ssr.enabled'); echo 'INERTIA_SSR_ENABLED=' . (\$$enabled ? 'true' : 'false') . PHP_EOL; } catch (Exception \$$e) { echo 'ERROR: ' . \$$e->getMessage() . PHP_EOL; }" || echo "⚠️  Config read failed"
	-$(DOCKER_COMPOSE_PROD) exec -T app php -r "try { \$$url = config('inertia.ssr.url'); echo 'INERTIA_SSR_URL=' . (\$$url ?? 'NOT SET') . PHP_EOL; } catch (Exception \$$e) { echo 'ERROR: ' . \$$e->getMessage() . PHP_EOL; }" || echo "⚠️  Config read failed"
	@echo ""
	...
	-$(DOCKER_COMPOSE_PROD) exec -T app php artisan config:cache || echo "⚠️  Config cache failed - check config files for errors"
	@echo ""
	@echo "✅ Production update complete."
	@echo ""
	@echo "If SSR is still not working, check your config/inertia.php file"
